        ...
        <form class="form-signin" method="POST" action = "<c:url value="/login"/>">
			...
			<c:choose>
				<c:when test="${userNameEntered != null}">
					<input type="text" name="username" class="form-control" value = "${userNameEntered}" maxlength="16" size="16" required/>
				</c:when>
				<c:otherwise>
					<input type="text" name="username" class="form-control" placeholder= "user name here" maxlength="16" size="16" required/>				
				</c:otherwise>
			</c:choose>
			
			<input type="password" name="password" class="form-control" placeholder = "password here" maxlength = "16" size = "16" required>
